...
@app.route('/')
def home():
    # Attempt to get Public IP from AWS Metadata Service
    try:
        public_ip = requests.get("http://169.254.169.254/latest/meta-data/public-ipv4", timeout=2).text
    except Exception:
        # Fallback if metadata is unreachable (common in some Docker network modes)
        public_ip = "Metadata Service Unavailable"

    # These are passed into the container via the 'docker run -e' command in user_data.sh
